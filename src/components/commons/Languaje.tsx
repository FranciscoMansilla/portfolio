import javascript from "../../assets/icons/javascript.png";

const Languaje = () => {
  return (
    <div className=" w-full flex flex-col items-center">
      <h6
        className="  delay-150 font-F-Logo text-3xl  bg-gradient-to-r to-sky-400 from-pink-300 text-transparent bg-clip-text
          hover:bg-gradient-to-r hover:to-violet-400 hover:from-pink-300  
      "
      >
        Languajes
      </h6>
      <div className="w-full xl:w-8/12 flex justify-center flex-wrap p-2">
        <div className="m-8">
          <img className="w-20 h-20 mx-2" src={javascript} alt="png" />
          <h6 className=" font-F-Medium text-lg">javascript</h6>
        </div>
        <div className="m-8">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 207 78"
            className=" w-28 h-20 mx-2"
          >
            <g fill="#000000" fill-rule="evenodd">
              <path d="m16.2 24.1c-.4 0-.5-.2-.3-.5l2.1-2.7c.2-.3.7-.5 1.1-.5h35.7c.4 0 .5.3.3.6l-1.7 2.6c-.2.3-.7.6-1 .6z" />
              <path d="m1.1 33.3c-.4 0-.5-.2-.3-.5l2.1-2.7c.2-.3.7-.5 1.1-.5h45.6c.4 0 .6.3.5.6l-.8 2.4c-.1.4-.5.6-.9.6z" />
              <path d="m25.3 42.5c-.4 0-.5-.3-.3-.6l1.4-2.5c.2-.3.6-.6 1-.6h20c.4 0 .6.3.6.7l-.2 2.4c0 .4-.4.7-.7.7z" />
              <g transform="translate(55)">
                <path d="m74.1 22.3c-6.3 1.6-10.6 2.8-16.8 4.4-1.5.4-1.6.5-2.9-1-1.5-1.7-2.6-2.8-4.7-3.8-6.3-3.1-12.4-2.2-18.1 1.5-6.8 4.4-10.3 10.9-10.2 19 .1 8 5.6 14.6 13.5 15.7 6.8.9 12.5-1.5 17-6.6.9-1.1 1.7-2.3 2.7-3.7-3.6 0-8.1 0-19.3 0-2.1 0-2.6-1.3-1.9-3 1.3-3.1 3.7-8.3 5.1-10.9.3-.6 1-1.6 2.5-1.6h36.4c-.2 2.7-.2 5.4-.6 8.1-1.1 7.2-3.8 13.8-8.2 19.6-7.2 9.5-16.6 15.4-28.5 17-9.8 1.3-18.9-.6-26.9-6.6-7.4-5.6-11.6-13-12.7-22.2-1.3-10.9 1.9-20.7 8.5-29.3 7.1-9.3 16.5-15.2 28-17.3 9.4-1.7 18.4-.6 26.5 4.9 5.3 3.5 9.1 8.3 11.6 14.1.6.9.2 1.4-1 1.7z" />
                <path
                  d="m107.2 77.6c-9.1-.2-17.4-2.8-24.4-8.8-5.9-5.1-9.6-11.6-10.8-19.3-1.8-11.3 1.3-21.3 8.1-30.2 7.3-9.6 16.1-14.6 28-16.7 10.2-1.8 19.8-.8 28.5 5.1 7.9 5.4 12.8 12.7 14.1 22.3 1.7 13.5-2.2 24.5-11.5 33.9-6.6 6.7-14.7 10.9-24 12.8-2.7.5-5.4.6-8 .9zm23.8-40.4c-.1-1.3-.1-2.3-.3-3.3-1.8-9.9-10.9-15.5-20.4-13.3-9.3 2.1-15.3 8-17.5 17.4-1.8 7.8 2 15.7 9.2 18.9 5.5 2.4 11 2.1 16.3-.6 7.9-4.1 12.2-10.5 12.7-19.1z"
                  fill-rule="nonzero"
                />
              </g>
            </g>
          </svg>
          <h6 className=" font-F-Medium text-lg">Go</h6>
        </div>
        <div className="m-8">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            className="icon icon-tabler w-20 h-20"
            viewBox="0 0 24 24"
            stroke-width="1.5"
            stroke="#2c3e50"
            fill="none"
            stroke-linecap="round"
            stroke-linejoin="round"
          >
            <path stroke="none" d="M0 0h24v24H0z" fill="none" />
            <path d="M12 9h-7a2 2 0 0 0 -2 2v4a2 2 0 0 0 2 2h3" />
            <path d="M12 15h7a2 2 0 0 0 2 -2v-4a2 2 0 0 0 -2 -2h-3" />
            <path d="M8 9v-4a2 2 0 0 1 2 -2h4a2 2 0 0 1 2 2v5a2 2 0 0 1 -2 2h-4a2 2 0 0 0 -2 2v5a2 2 0 0 0 2 2h4a2 2 0 0 0 2 -2v-4" />
            <path d="M11 6l0 .01" />
            <path d="M13 18l0 .01" />
          </svg>
          <h6 className=" font-F-Medium text-lg">python</h6>
        </div>
        <div className="m-8">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            className="w-20 h-20"
            viewBox="0 0 40 55"
            fill="#000000"
          >
            <path
              d="M27.3398 37.7219C25.9985 38.1017 22.9773 38.7211 18.6571 38.7211C14.4173 38.7211 10.953 37.9953 10.9422 37.1406C10.935 36.5721 11.6224 36.3243 11.6224 36.3243L11.5001 36.2533C9.46984 36.6122 7.58382 37.1669 7.59472 37.9974C7.61352 39.5044 13.3765 40.6362 18.6488 40.6362C23.131 40.6362 27.4358 39.8841 29.3776 38.8989L27.3398 37.7219ZM13.0118 42.381C12.0702 42.5684 10.0181 43.0396 10.0181 44.0375C10.0181 45.4198 14.4086 46.4785 18.6471 46.4785C24.4794 46.4785 26.8682 44.9778 26.9763 44.9001L24.5512 43.4971C23.5199 43.7428 21.7821 44.1289 18.6529 44.1289C15.1606 44.1289 12.8853 43.5308 12.8853 42.876C12.8853 42.737 12.9721 42.5722 13.134 42.4517L13.0118 42.381ZM36.8447 33.0803C36.6815 36.2104 33.7897 38.1593 30.8993 39.8061L31.1614 39.9572C34.2455 39.0896 39.7485 36.5629 39.2936 32.6792C39.067 30.7424 37.2967 29.3585 34.9888 29.3585C34.2701 29.3585 33.6307 29.4853 33.1124 29.6431L33.1108 29.6481L33.0014 29.9236C35.0656 29.5196 36.9519 31.027 36.8447 33.0803V33.0803ZM18.5674 51.449C26.6804 51.3785 35.7579 49.7905 35.7345 47.1216C35.7303 46.6379 35.4153 46.3066 35.1415 46.1063L35.0084 46.1826C34.2584 48.2501 27.919 49.7776 18.5536 49.8593C12.5105 49.912 4.13956 48.4642 4.12492 46.7911C4.11029 45.1138 8.09045 44.192 8.09045 44.192L7.80925 44.0313C5.14203 44.3985 0.227453 45.6759 0.243692 47.5202C0.26709 50.1866 11.562 51.5104 18.5674 51.449V51.449ZM17.7234 53.3545C14.4987 53.383 10.5611 53.1159 7.24867 52.5591L6.94194 52.7356C10.2414 53.7031 14.8351 54.2874 19.8828 54.2435C29.7959 54.1567 37.8329 51.6985 37.9919 48.7401L37.8771 48.6733C37.2127 49.4842 32.9251 53.2219 17.7234 53.3545V53.3545ZM5.30474 32.3666C5.30474 30.8713 10.9768 30.0324 13.6177 29.8292L13.8698 29.9748C12.8552 30.1594 8.7849 30.8772 8.7849 31.8136C8.7849 32.8332 15.0279 33.5014 18.6517 33.5014C24.8065 33.5014 28.9903 32.5698 30.1155 32.2627L31.6897 33.1788C30.6117 33.7067 25.9835 35.0902 18.6521 35.0902C10.5039 35.0902 5.30474 33.4959 5.30474 32.3666"
              fill="#000000"
            />
            <path
              d="M29.9277 11.4664L29.7913 11.3884C27.3131 12.2184 19.6854 15.2291 19.6854 20.8427C19.6854 24.0178 22.7858 25.776 22.7858 28.7534C22.7858 29.8155 22.1861 30.8112 21.6979 31.4064L21.9433 31.5483C23.2337 30.7094 25.5185 28.8931 25.5185 26.5481C25.5185 24.5624 22.77 22.1765 22.77 19.624C22.77 15.6022 28.0731 12.4467 29.9277 11.4664V11.4664ZM26.1834 4.10872C26.1834 12.417 14.7976 15.5963 14.7976 21.5033C14.7976 25.65 17.5474 28.2505 19.0686 29.8943L18.945 29.9657C17.0244 28.7642 11.9709 25.7448 11.9709 20.7572C11.9709 13.7551 25.0486 10.4092 25.0486 2.4556C25.0486 1.47696 24.9038 0.726955 24.8024 0.322143L24.9347 0.245862C25.3491 0.7667 26.1834 2.06874 26.1834 4.10872"
              fill="#000000"
            />
          </svg>
          <h6 className=" font-F-Medium text-lg">java</h6>
        </div>
      </div>
    </div>
  );
};
export default Languaje;
